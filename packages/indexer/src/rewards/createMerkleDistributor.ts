...
const MIN_BALANCE_FOR_REWARD = new BigNumber(0.05 * WEI_PER_UNIT)
...
export async function createAndDeployMerkleDistributor(fromBlock: number, toBlock: number) {
  ...
  const distributionInfo = await deployDistributor(merkleTree)
  merkleTree.contractAddress = distributionInfo.contractAddress
...
async function outputToFileAndUpload(directory: string, fileName: string, output: any) {
  ...
  await uploadFile('reward-distributions', `${directory}/${fileName}`, output)
}
...
async function calculateRewardsByAddress(fromBlock: number, toBlock: number) {
  ...
  for (const identifier of Object.keys(accountsByIdentifier)) {
    ...
    if (bestAddress && bestAverageBalance && bestAverageBalance.gt(MIN_BALANCE_FOR_REWARD)) {
